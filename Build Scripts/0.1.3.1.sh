...
git checkout -f dc976d756f9d3273c3c6f960fadb88e44b468050
...
if [ -d "$BUILD_DIRECTORY/build/$PACKAGE_VERSION/$BUILD_MODE" ]; then
    echo "\033[0;36m▸ Delete old build artefacts\033[0m"
    rm -rf $BUILD_DIRECTORY/build/$PACKAGE_VERSION/$BUILD_MODE
fi
...
xcodebuild -create-xcframework \
    ...
    -headers $HELIOS_DIRECTORY/headers \
    -output $BUILD_DIRECTORY/build/$PACKAGE_VERSION/$BUILD_MODE/helios.xcframework

if [ $BUILD_MODE = "release" ]; then
    echo "\033[0;36m▸ Compress helios.xcframework\033[0m"
    ditto -c -k --sequesterRsrc --keepParent $BUILD_DIRECTORY/build/$PACKAGE_VERSION/$BUILD_MODE/helios.xcframework $BUILD_DIRECTORY/build/$PACKAGE_VERSION/$BUILD_MODE/helios.xcframework.zip

    echo "\033[0;36m▸ Compute helios.xcframework checksum\033[0m"
    swift package compute-checksum $BUILD_DIRECTORY/build/$PACKAGE_VERSION/$BUILD_MODE/helios.xcframework.zip > $BUILD_DIRECTORY/build/$PACKAGE_VERSION/$BUILD_MODE/helios.xcframework.zip.checksum
fi

echo "\033[0;36m▸ Copy Swift bridging files\033[0m"
cp $HELIOS_DIRECTORY/generated/SwiftBridgeCore.swift $BUILD_DIRECTORY/build/$PACKAGE_VERSION/$BUILD_MODE/SwiftBridgeCore.swift
sed -i '' '1i\'$'\n''import helios'$'\n' $BUILD_DIRECTORY/build/$PACKAGE_VERSION/$BUILD_MODE/SwiftBridgeCore.swift
sed -i '' 's/public //' $BUILD_DIRECTORY/build/$PACKAGE_VERSION/$BUILD_MODE/SwiftBridgeCore.swift
cp $HELIOS_DIRECTORY/generated/helios/helios.swift $BUILD_DIRECTORY/build/$PACKAGE_VERSION/$BUILD_MODE/helios.swift
sed -i '' '1i\'$'\n''import helios'$'\n' $BUILD_DIRECTORY/build/$PACKAGE_VERSION/$BUILD_MODE/helios.swift
sed -i '' 's/public //' $BUILD_DIRECTORY/build/$PACKAGE_VERSION/$BUILD_MODE/helios.swift

echo "\033[0;32m▸ BUILD SUCCESSFUL!"
echo ''
echo "Built artefacts can be found at $BUILD_DIRECTORY/build/$PACKAGE_VERSION\033[0m"
