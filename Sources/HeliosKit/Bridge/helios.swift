...
struct Log {
    ...
    func intoFfiRepr() -> __swift_bridge__$Log {
        { let val = self; return __swift_bridge__$Log(address: { let rustString = val.address.intoRustString(); rustString.isOwned = false; return rustString.ptr }(), topics: { let val = val.topics; val.isOwned = false; return val.ptr }(), data: { let val = val.data; val.isOwned = false; return val.ptr }(), block_hash: { let rustString = val.block_hash.intoRustString(); rustString.isOwned = false; return rustString.ptr }(), block_number: { let val = val.block_number; return __private__OptionU64(val: val ?? 123, is_some: val != nil); }(), transaction_hash: { let rustString = val.transaction_hash.intoRustString(); rustString.isOwned = false; return rustString.ptr }(), transaction_index: { let val = val.transaction_index; return __private__OptionU64(val: val ?? 123, is_some: val != nil); }(), log_index: { let rustString = val.log_index.intoRustString(); rustString.isOwned = false; return rustString.ptr }(), transaction_log_index: { let rustString = val.transaction_log_index.intoRustString(); rustString.isOwned = false; return rustString.ptr }(), log_type: { let rustString = val.log_type.intoRustString(); rustString.isOwned = false; return rustString.ptr }(), removed: { let val = val.removed; return __private__OptionBool(val: val ?? false, is_some: val != nil); }()); }()
    }
}
extension __swift_bridge__$Log {
    @inline(__always)
    func intoSwiftRepr() -> Log {
        { let val = self; return Log(address: RustString(ptr: val.address), topics: RustVec(ptr: val.topics), data: RustVec(ptr: val.data), block_hash: RustString(ptr: val.block_hash), block_number: { let val = val.block_number; if val.is_some { return val.val } else { return nil } }(), transaction_hash: RustString(ptr: val.transaction_hash), transaction_index: { let val = val.transaction_index; if val.is_some { return val.val } else { return nil } }(), log_index: RustString(ptr: val.log_index), transaction_log_index: RustString(ptr: val.transaction_log_index), log_type: RustString(ptr: val.log_type), removed: { let val = val.removed; if val.is_some { return val.val } else { return nil } }()); }()
    }
}
...
struct TransactionReceipt {
    ...
    func intoFfiRepr() -> __swift_bridge__$TransactionReceipt {
        { let val = self; return __swift_bridge__$TransactionReceipt(transaction_hash: { let rustString = val.transaction_hash.intoRustString(); rustString.isOwned = false; return rustString.ptr }(), transaction_index: val.transaction_index, block_hash: { let rustString = val.block_hash.intoRustString(); rustString.isOwned = false; return rustString.ptr }(), block_number: { let val = val.block_number; return __private__OptionU64(val: val ?? 123, is_some: val != nil); }(), from: { let rustString = val.from.intoRustString(); rustString.isOwned = false; return rustString.ptr }(), to: { let rustString = val.to.intoRustString(); rustString.isOwned = false; return rustString.ptr }(), cumulative_gas_used: { let rustString = val.cumulative_gas_used.intoRustString(); rustString.isOwned = false; return rustString.ptr }(), gas_used: { let rustString = val.gas_used.intoRustString(); rustString.isOwned = false; return rustString.ptr }(), contract_address: { let rustString = val.contract_address.intoRustString(); rustString.isOwned = false; return rustString.ptr }(), logs: { let val = val.logs; val.isOwned = false; return val.ptr }(), status: { let val = val.status; return __private__OptionU64(val: val ?? 123, is_some: val != nil); }(), root: { let rustString = val.root.intoRustString(); rustString.isOwned = false; return rustString.ptr }(), logs_bloom: { let val = val.logs_bloom; val.isOwned = false; return val.ptr }(), transaction_type: { let val = val.transaction_type; return __private__OptionU64(val: val ?? 123, is_some: val != nil); }(), effective_gas_price: { let rustString = val.effective_gas_price.intoRustString(); rustString.isOwned = false; return rustString.ptr }()); }()
    }
}
extension __swift_bridge__$TransactionReceipt {
    @inline(__always)
    func intoSwiftRepr() -> TransactionReceipt {
        { let val = self; return TransactionReceipt(transaction_hash: RustString(ptr: val.transaction_hash), transaction_index: val.transaction_index, block_hash: RustString(ptr: val.block_hash), block_number: { let val = val.block_number; if val.is_some { return val.val } else { return nil } }(), from: RustString(ptr: val.from), to: RustString(ptr: val.to), cumulative_gas_used: RustString(ptr: val.cumulative_gas_used), gas_used: RustString(ptr: val.gas_used), contract_address: RustString(ptr: val.contract_address), logs: RustVec(ptr: val.logs), status: { let val = val.status; if val.is_some { return val.val } else { return nil } }(), root: RustString(ptr: val.root), logs_bloom: RustVec(ptr: val.logs_bloom), transaction_type: { let val = val.transaction_type; if val.is_some { return val.val } else { return nil } }(), effective_gas_price: RustString(ptr: val.effective_gas_price)); }()
    }
}
